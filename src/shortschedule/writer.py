...
class XMLWriter:
    ...
    def write_calendar(self, calendar, output_path=None, mission_phase='TST', 
                    revision=1, verbose=False):
        """
        ...
        if output_path is None:
            output_path = self._generate_filename(calendar, mission_phase, revision)
        ...
        if verbose:
            total_sequences = sum(len(visit.sequences) for visit in calendar.visits)  # FIXED LINE
            print(f"Written {len(calendar.visits)} visits with {total_sequences} sequences")
    ...
    def _generate_filename(self, calendar, mission_phase, revision):
        ...
        try:
            if valid_from:
                ...
                if 'T' in valid_from:
                    vf_date = datetime.fromisoformat(valid_from.replace('Z', '+00:00'))
                else:
                    vf_date = datetime.strptime(valid_from, '%Y-%m-%d %H:%M:%S')
        ...
        try:
            if expires:
                # Handle different date formats
                if 'T' in expires:
                    ex_date = datetime.fromisoformat(expires.replace('Z', '+00:00'))
                else:
    ...
    def _pretty_print_xml(self, file_path):
        ...
        try:
            ...
            for line in lines:
                ...
                if line.strip() or (not filtered_lines):  # Keep first line (XML declaration)
