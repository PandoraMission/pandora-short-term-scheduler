...
def test_validate_no_overlaps_astropy_detects_overlap():
    ...
    # seq1: 0 -> 120s
    seq1 = ObservationSequence(id='a1', target='A', priority=1, start_time=start,
                               stop_time=start + TimeDelta(120, format='sec'),
                               ra=0.0, dec=0.0, payload_params={})
    # seq2: starts at 60s -> overlap with seq1
    seq2 = ObservationSequence(id='a2', target='B', priority=1, start_time=start + TimeDelta(60, format='sec'),
...
def test_get_minute_by_minute_assignments_correctness():
    ...
    start = Time("2025-01-01T00:00:00", scale="utc")
    s1 = ObservationSequence(id='s1', target='T1', priority=1, start_time=start,
                             stop_time=start + TimeDelta(2 * 60, format='sec'),
                             ra=0.0, dec=0.0, payload_params={})
    s2 = ObservationSequence(id='s2', target='T2', priority=1, start_time=start + TimeDelta(2 * 60, format='sec'),
...
def test_validate_sequence_timing_flags_short_sequence():
    ...
    start = Time("2025-01-01T00:00:00", scale="utc")
    short_seq = ObservationSequence(id='short', target='S', priority=1,
                                    start_time=start, stop_time=start + TimeDelta(30, format='sec'),
                                    ra=0.0, dec=0.0, payload_params={})
    ...
    assert issues['short_sequences'], "Expected short_sequences to be non-empty"
