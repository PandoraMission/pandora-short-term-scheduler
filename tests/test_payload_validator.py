...
from shortschedule.models import ObservationSequence, Visit, ScienceCalendar
...
def test_validator_detects_total_exposure_over_duration():
    ...
    payload = {'AcquireVisCamScienceData': make_vda_element(1_000_000, num_frames=20)}
    seq = ObservationSequence(id='s1', target='T', priority=1, start_time=start, stop_time=stop,
                              ra=0.0, dec=0.0, payload_params=payload)
    ...
    assert any(i.get('problem') == 'total_exposure_longer_than_sequence' for i in issues)
...
def test_validator_detects_single_exposure_over_duration():
    ...
    seq = ObservationSequence(id='s2', target='U', priority=1, start_time=start, stop_time=stop,
                              ra=0.0, dec=0.0, payload_params=payload)
    ...
    sched = ScheduleProcessor.__new__(ScheduleProcessor)
    issues = sched.validate_payload_exposures(cal, report_issues=False)
    assert any(i.get('problem') == 'single_exposure_longer_than_sequence' for i in issues)
...
def test_validator_accepts_valid_exposure():
    ...
    payload = {'AcquireVisCamScienceData': make_vda_element(5_000_000, num_frames=3)}
    seq = ObservationSequence(id='s3', target='V', priority=1, start_time=start, stop_time=stop,
                              ra=0.0, dec=0.0, payload_params=payload)
...
def test_validator_checks_frames_per_coadd_when_no_total_frames():
    ...
    payload = {'AcquireVisCamScienceData': make_vda_element(2_000_000, frames_per_coadd=3)}
    seq = ObservationSequence(id='s4', target='W', priority=1, start_time=start, stop_time=stop,
                              ra=0.0, dec=0.0, payload_params=payload)
    ...
    sched = ScheduleProcessor.__new__(ScheduleProcessor)
    issues = sched.validate_payload_exposures(cal, report_issues=False)
    assert any(i.get('problem') == 'coadd_exposure_longer_than_sequence' for i in issues)
...
def test_validator_heuristic_flat_field_detection_and_non_numeric_robustness():
    ...
    seq = ObservationSequence(id='s5', target='X', priority=1, start_time=start, stop_time=stop,
                              ra=0.0, dec=0.0, payload_params=payload)
    ...
    field_issues = [i for i in issues if i.get('problem') == 'payload_exposure_field_longer_than_sequence']
