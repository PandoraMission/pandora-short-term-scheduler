## v1.5.0 (2026-X)
- Runs on `pandoravisibility` v2.0.0. That release folded `get_best_roll`, `get_visibility_best_roll` and `get_orbit_roll_angles` into `get_visibility`, which now returns a details dict (`visible`, `boresight_visible`, `roll_deg`, `n_visible`, `n_st_pass`, `solar_power_frac`) instead of a boolean array. Every visibility call in the scheduling passes, `validate_visibility`, the visibility Gantt and the sequence debug printout reads the `visible` entry, and `get_best_roll_per_visit` runs the roll search through `get_visibility(optimize_roll=True, ...)`. Upstream checked both the `visible` array and the searched roll against v1.4.0 step for step, so delivered calendars do not change.
  - `get_best_roll_per_visit` still reports `roll_deg` as one number per target. Upstream now echoes the held roll once per timestep; the visit rule collapses that, since one roll serves the whole visit.
  - A visibility object handed to `ScheduleProcessor` in place of `Visibility` must now return that dict from `get_visibility` and accept `optimize_roll`, `roll_step`, `min_power_frac` and `weights`; `get_best_roll` is no longer called. The test doubles do this through `tests/doubles.answers_visibility`, a decorator on a boolean `get_visibility` that replaced the `BestRollFromVisibility` mixin.
